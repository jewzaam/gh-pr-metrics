...
def _block_http(monkeypatch):
    """Block outbound network connections — no real API calls in tests.

    Patches at socket level so requests_mock (which intercepts before
    reaching the socket) still works, but un-mocked requests fail.
    """

    def _blocked(*args, **kwargs):
        raise RuntimeError("Network blocked by conftest. Use requests_mock instead.")
...
def isolate_state_file(tmp_path, monkeypatch):
    ...
    gh_pr_metrics.state_manager = gh_pr_metrics.StateManager(
        logger=gh_pr_metrics.logger
    )
